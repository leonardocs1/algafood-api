...
public interface GrupoPermissaoControllerOpenApi {
	...
	@ApiOperation("Associação de permissão a um grupo")
	@ApiResponses({
	...
	@ApiOperation("Desassociação de permissão a um grupo")
	@ApiResponses({
		@ApiResponse(code = 204, message = "Desassociação realizada com sucesso"),
		@ApiResponse(code = 404, message = "Grupo ou permissão não encontrada", response = Problem.class)
	})
	ResponseEntity<Void> dessasociar(@ApiParam(value = "ID do grupo", example = "1", required = true) Long grupoId,
			@ApiParam(value = "ID da permissão", example = "1", required = true) Long permissaoId);
